...
class resblock(nn.Module):
    def __init__(self, dim):
        ...
        if depthwise_separable:
            self.body = nn.Sequential(depthwise_separable_conv2d(dim, dim, kernel_size=3, stride=1, padding=1, bias=False),
                                    nn.PReLU(),
                                    depthwise_separable_conv2d(dim, dim, kernel_size=3, stride=1, padding=1, bias=False))
        else:
...
class Downsample(nn.Module):
    def __init__(self, n_feat):
        ...
        if depthwise_separable:
            self.body = nn.Sequential(depthwise_separable_conv2d(n_feat, n_feat//2, kernel_size=3, stride=1, padding=1, bias=False),
                                    nn.PixelUnshuffle(2))
        else:
...
class OverlapPatchEmbed(nn.Module):
    def __init__(self, in_c=3, embed_dim=48, bias=False):
        ...
        if depthwise_separable:
            self.proj = depthwise_separable_conv2d(in_c, embed_dim, kernel_size=3, stride=1, padding=1, bias=bias)
        else:
...
class PromptGenBlock(nn.Module):
    def __init__(self,prompt_dim=128,prompt_len=5,prompt_size = 96,lin_dim = 192):
        ...
        if depthwise_separable:
            self.conv3x3 = depthwise_separable_conv2d(prompt_dim,prompt_dim,kernel_size=3,stride=1,padding=1,bias=False)
        else:
...
class PromptIR(nn.Module):
    def __init__(self,
        ...
        global depthwise_separable, instance_normalize
        depthwise_separable = depthwise_separable_enable
        instance_normalize = instance_normalize_enable
        ...
        if depthwise_separable:
            self.output = depthwise_separable_conv2d(int(dim*2**1), out_channels, kernel_size=3, stride=1, padding=1, bias=bias)
        else:
